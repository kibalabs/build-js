...
export const runLinting = async (inputParams = {}) => {
  ...
  if (!params.skipStylelint) {
    ...
    const stylelintResult = await stylelint.lint({
      files: [`${workingDirectory}/src/**/*.css`, `${workingDirectory}/src/**/*.scss`],
...
const runEslint = async (params, workingDirectory) => {
  ...
  const eslintResults = await eslint.lintFiles([`${workingDirectory}/**/*.js`, `${workingDirectory}/**/*.jsx`, `${workingDirectory}/**/*.ts`, `${workingDirectory}/**/*.tsx`]);
  if (params.fix) {
  ...
  return [
    {
      label: 'ESLint',
      diagnostics: eslintResultsToDiagnostics(eslintResults),
      fixableErrorCount,
      fixableWarningCount,
    },
  ];
...
const runNodeBin = async (packageName, args) => {
  ...
  try {
    ...
    // but a real crash (e.g. an invalid config) also exits non-zero, so `error.stdout` alone can't tell
    // the two apart. oxfmt reports crashes on stderr with empty stdout, so any stderr content here means
    // the tool never produced real results and should be surfaced, not treated as "0 issues found".
    if (error.stderr) {
      throw new Error(`${packageName} failed to run:\n${error.stderr}`);
    }
